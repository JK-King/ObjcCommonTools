...
#import "EncryptionRSA.h"
#import <Security/Security.h>

@implementation EncryptionRSA
...
+ (SecKeyRef)addPublicKey:(NSString *)key{
    ...
    data = [EncryptionRSA stripPublicKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (SecKeyRef)addPrivateKey:(NSString *)key{
    ...
    // This will be base64 encoded, decode it.
    NSData *data = base64_decode(key);
    data = [EncryptionRSA stripPrivateKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (NSData *)jk_encryptData:(NSData *)data privateKey:(NSString *)privKey{
    ...
    SecKeyRef keyRef = [EncryptionRSA addPrivateKey:privKey];
    if(!keyRef){
        return nil;
    }
    return [EncryptionRSA encryptData:data withKeyRef:keyRef isSign:YES];
}
...
+ (NSData *)jk_encryptData:(NSData *)data publicKey:(NSString *)pubKey{
    ...
    SecKeyRef keyRef = [EncryptionRSA addPublicKey:pubKey];
    if(!keyRef){
        return nil;
    }
